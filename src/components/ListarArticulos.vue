<template>
    <v-container>
      ...
      <v-row>
        <v-col class="mb-1">
          <v-btn :to="{ name: 'crearArticulo' }" class="mx-2" fab red style="background-color: rgb(201, 41, 41);">
            <v-icon dark style="color: rgb(253, 253, 253);">mdi-plus</v-icon>
          </v-btn>
        </v-col>
      </v-row>
    ...
    <v-col cols="12">
        ...
        <tbody>
            <tr v-for="articulo in articulos" :key="articulo.id">
                ...
                <td>
                    <v-btn :to="{name:'editarArticulo', params:{id:articulo.id}}" fab small color="primary"><v-icon dark style="color: rgb(29, 147, 216);">mdi-table-edit</v-icon></v-btn>
                    <v-btn @click.stop="dialog=true" @click="id=articulo.id" fab small color="error"><v-icon dark style="color: rgb(241, 45, 45);">mdi-archive</v-icon></v-btn>
                </td>
            </tr>
         </tbody>   
        </template>
